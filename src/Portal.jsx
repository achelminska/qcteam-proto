...
function IntegrationsPage({ s, set }) {
  ...
  const fetchPushed = async target => { if (!window.__qcServer) { patchIt({ liveStatus: "No state server — run `npm run server` locally." }); return; } patchIt({ liveStatus: "Fetching…" }); try { const r = await fetch(`${window.__qcServer}/sheet/${target.purpose === "Dock" ? "dock" : "products"}`); if (r.status === 404) { patchIt({ liveStatus: "Nothing pushed yet — the sheet's script hasn't sent data (run pushToQCteam once in Apps Script)." }); return; } const raw = await r.json(); let j = detectTable({ header: raw.header, rows: raw.rows }); const mappings = target.mappings.length && target.header.join("|") === j.header.join("|") ? target.mappings : suggestMappings(j.header, j.rows, targets); const next = { ...target, header: j.header, sample: j.rows, mappings }; const rows = applyMapping(next, j.header, j.rows); patchIt({ header: j.header, sample: j.rows, rawHeader: raw.header, rawRows: raw.rows, mappings, rows: target.purpose === "Dock" ? rows : target.rows, summary: target.purpose === "Dock" ? extractSummary(raw.header, raw.rows) : target.summary, lastSyncAt: nowISO(), liveStatus: `OK — ${j.rows.length} rows, pushed by the sheet at ${new Date(raw.receivedAt).toLocaleTimeString("en-GB")}` }); } catch (e) { patchIt({ liveStatus: `Could not read pushed data: ${e.message}` }); } };
...
function MessagesPage({ s, set, user }) {
          ...
          {!conv ? <Empty icon="💬" title="Select a conversation" hint="Or start a new one on the left." /> : (
            <>
              ...
              <div className="rounded-lg p-3 mb-2 overflow-y-auto" style={{ background: C.bg, minHeight: 220, maxHeight: 360 }}>
                ...
                {(conv.messages || []).map(m => { const me = m.senderId === user.id; return <div key={m.id} className={`flex mb-1.5 ${me ? "justify-end" : "justify-start"}`}><div className="rounded-xl px-3 py-1.5 max-w-[75%]" style={{ background: me ? C.accent : C.surface, color: me ? C.onDark : C.ink, border: me ? "none" : `1px solid ${C.line}` }}>{!me && conv.isGroup && <p className="text-[10px] font-medium" style={{ color: C.accent }}>{s.users.find(u => u.id === m.senderId)?.name}</p>}{m.productId && <p className="text-[10px] mb-0.5 px-1.5 py-0.5 rounded inline-block" style={{ background: me ? C.onDarkSoft : C.accentSoft, color: me ? C.onDark : C.accent }}>📦 {s.products.find(p => p.id === m.productId)?.name || "product"}</p>}<p className="text-sm">{m.text}</p><p className="text-[10px] mt-0.5" style={{ color: me ? C.onDarkMuted : C.muted }}>{fmtTime(m.at)}</p></div></div>; })}
              </div>
              <div className="flex gap-1.5"><button className="text-xs px-2 rounded" style={{ background: C.line, color: C.muted }} title="MessageAttachment — attachment"><Ic i={Paperclip} s={15} mr={0} /></button><input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="message…" className="flex-1 text-sm rounded px-2 py-1.5 outline-none" style={inp} /><Primary small onClick={send}>Send</Primary></div>
            </>
          )}
        </Card>
      </div>
...
export default function App() {
  ...
  return (
    <Shell page={safePage} setPage={setPage} badge={{ ...badge, messages: unreadMsgs, notifications: unread, flags: user.role === "Head" ? s.flags.filter(f => f.status === "Open").length : 0, inspections: user.role === "Head" ? s.inspections.filter(i => i.status === "PendingReview").length : 0 }} topRight={dataButton} users={s.users} user={user} setUser={id => { setUserId(id); setPage("dashboard"); setOpenInspId(null); }} unread={unread} onBell={() => setPage("notifications")}>
      ...
      {safePage === "messages" && <MessagesPage s={s} set={set} user={user} />}
    </Shell>
  );
}
