...
const suggestTarget = (col, targets) => { const c = col.toLowerCase().replace(/[^a-z0-9]/g, ""); const table = { hu: ["handlingunit", "hu", "sscc", "pallet"], article: ["uomid", "uom", "article", "sku", "articleid"], articleId: ["uomid", "uom", "article", "sku", "articleid"], name: ["itemname", "name", "product", "omschrijving"], location: ["location", "locatie", "dock"], priority: ["priorityitem", "priority"], blocking: ["neededtoday", "needed", "urgent"], skippable: ["skippable", "skip"], arrived: ["arrivaldate", "arrival", "date", "datum"], arrivedTime: ["arrivaltime", "time"], transporter: ["transporter", "carrier"], po: ["poid", "po", "order"], cusPerTu: ["cupertu", "cus", "cu"], sortable: ["sortable", "sort"], barcode: ["barcode", "ean"], piecesPerCu: ["piecespercu", "pieces", "stuks"], weightPerCu: ["weightpercu", "weight", "gewicht"], category: ["category", "categorie"] }; for (const [k] of targets) { if ((table[k] || []).some(alias => c === alias)) return k; } for (const [k] of targets) { if ((table[k] || []).some(alias => c.includes(alias) && alias.length > 2)) return k; } return "ignore"; };
const ALIASES = { hu: ["handlingunit", "hu", "sscc", "pallet", "palletid"], article: ["uomid", "uom", "articleid", "article", "sku", "artikel"], articleId: ["uomid", "uom", "articleid", "article", "sku", "artikel"], name: ["itemname", "productname", "name", "product", "omschrijving", "item"], location: ["location", "locatie"], priority: ["priorityitem", "priority", "prioriteit"], blocking: ["neededtoday", "urgent"], skippable: ["skippable", "skip"], arrived: ["arrivaldate", "arrival", "date", "datum"], arrivedTime: ["arrivaltime", "time", "tijd"], transporter: ["transporter", "carrier", "vervoerder"], po: ["poid", "po", "order", "ponumber"], cusPerTu: ["cupertu", "cus", "cu"], sortable: ["sortable", "sorteerbaar"], barcode: ["barcode", "ean", "gtin"], piecesPerCu: ["piecespercu", "pieces", "stuks"], weightPerCu: ["weightpercu", "weight", "gewicht"], category: ["category", "categorie"] };
...
const suggestTransform = (target, sample) => target === "article" || target === "articleId" ? (/^[A-Z]+\d+-\d+$/.test(String(sample || "").trim()) ? "uom_article" : "none") : target === "cusPerTu" ? (/-\d+$/.test(String(sample || "").trim()) ? "uom_cus" : "number") : target === "blocking" || target === "skippable" || target === "sortable" ? "yesno" : target === "arrived" ? "date_dmy" : target === "priority" ? "priority" : "none";
...
const detectTable = ({ header, rows }) => {
  ...
  const h = all[best].map(c => String(c).trim()); let width = h.length; while (width > 0 && !h[width - 1]) width--;
...
const SUMMARY_LABELS = { urgentPallets: /^urgent pallets on dock/i, nonUrgentPallets: /^non urgent pallets on dock/i, urgentSkus: /^urgent sku on dock/i, skus: /^sku on dock/i, expected: /^sku still expected/i, skippableSkus: /^skus:?$/i, skippablePallets: /^pallets:?$/i };
const extractSummary = (header, rows) => {
  const all = [header, ...rows]; const out = {}; let skippableMode = false;
  all.forEach(r => { r.forEach((c, i) => { const t = String(c || "").trim(); if (!t) return; if (/^skippable$/i.test(t)) { skippableMode = true; return; }
...
function IntegrationsPage({ s, set }) {
  ...
  const targets = it?.purpose === "Products" ? PRODUCT_TARGETS : DOCK_TARGETS;
  const patchIt = ch => set(x => ({ ...x, integrations: x.integrations.map(i => i.id === sel ? { ...i, ...ch } : i) }));
  const create = purpose => { const id = uid(); set(x => ({ ...x, integrations: [...(x.integrations || []), { id, name: purpose === "Dock" ? "Dock sheet" : "Product profiles sheet", purpose, sourceUrl: "", header: [], sample: [], mappings: [], rows: [], lastSyncAt: null, lastError: null }] })); setSel(id); };
  ...
  const fetchLive = async target => { if (!target?.sourceUrl) return; patchIt({ liveStatus: "Fetching…" }); try { let r, txt; try { r = await fetch(target.sourceUrl); txt = await r.text(); } catch (direct) { if (!window.__qcServer) throw direct; r = await fetch(`${window.__qcServer}/proxy?url=${encodeURIComponent(target.sourceUrl)}`); if (!r.ok) throw new Error(`proxy ${r.status}: ${await r.text()}`); txt = await r.text(); } let j; try { j = JSON.parse(txt); } catch { j = txt.includes("\t") && !txt.includes(",") ? parseTsv(txt) : parseCsv(txt); } if (!Array.isArray(j.header) || !Array.isArray(j.rows)) throw new Error("unexpected response — expected JSON {header, rows} or CSV"); const raw = { header: j.header, rows: j.rows }; j = detectTable(j); const mappings = target.mappings.length && target.header.join("|") === j.header.join("|") ? target.mappings : suggestMappings(j.header, j.rows, targets); const next = { ...target, header: j.header, sample: j.rows, mappings }; const rows = applyMapping(next, j.header, j.rows); patchIt({ header: j.header, sample: j.rows, rawHeader: raw.header, rawRows: raw.rows, mappings, rows: target.purpose === "Dock" ? rows : target.rows, summary: target.purpose === "Dock" ? extractSummary(raw.header, raw.rows) : target.summary, lastSyncAt: nowISO(), liveStatus: `OK — ${j.rows.length} rows at ${new Date().toLocaleTimeString("en-GB")}` }); } catch (e) { const local = /^(localhost|127\.|10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.)/.test(location.hostname); patchIt({ liveStatus: local ? `Could not fetch (${e.message}). Most often the Apps Script deployment is not public: open the /exec link in an incognito window — if Google asks you to sign in, redeploy with “Who has access: Anyone”. Also make sure you copied the /exec link, not /dev.` : `Could not fetch (${e.message}). The Claude sandbox blocks cross-origin requests — run the prototype locally to test live.` }); } };
  useEffect(() => { if (!it?.autoRefresh || !it?.sourceUrl) return; const id = setInterval(() => fetchLive(it), 60000); return () => clearInterval(id); }, [it?.id, it?.autoRefresh, it?.sourceUrl]);
    ...
    <div>
      ...
      <div className="flex gap-4 items-start">
        <aside className="w-56 flex-shrink-0">
          {list.map(i => <button key={i.id} onClick={() => setSel(i.id)} className="w-full text-left text-sm px-3 rounded-xl mb-1 flex items-center gap-2" style={{ height: 40, background: sel === i.id ? C.accentSoft : "transparent", color: sel === i.id ? C.accent : C.ink }}><Ic i={i.purpose === "Dock" ? Truck : Package} s={15} mr={0} /><span className="flex-1 truncate">{i.name}</span>{i.rows?.length ? <span className="text-[10px]" style={{ color: C.muted }}>{i.rows.length}</span> : null}</button>)}
          <div className="mt-3 flex flex-col gap-1"><button onClick={() => create("Dock")} className="text-xs text-left px-3 py-1.5" style={{ color: C.accent }}>+ dock sheet (pallets)</button><button onClick={() => create("Products")} className="text-xs text-left px-3 py-1.5" style={{ color: C.accent }}>+ product profiles sheet</button></div>
        </aside>
        <div className="flex-1 min-w-0">
          {!it ? <Card><Empty icon="📋" title="No integrations yet" hint="Add the dock sheet first — it feeds the dashboard, the scanner and same-delivery pallets." /></Card> : <>
            <Card style={{ marginBottom: 16 }}>
              <div className="flex items-center gap-2 mb-2"><input value={it.name} onChange={e => patchIt({ name: e.target.value })} className="font-semibold text-sm flex-1" /><span className="text-[11px] px-2 py-0.5 rounded" style={{ background: C.bg, color: C.muted }}>{it.purpose}</span></div>
  ...
  return ContentService.createTextOutput(JSON.stringify({ header, rows: body.map(r => r.map(String)) }))
    .setMimeType(ContentService.MimeType.JSON);
              ...
              <p className="label-sm mb-1">1 · Source</p>
            ...
            {it.header.length > 0 && (
              <Card>
                {it.purpose === "Dock" && (() => { const sm = extractSummary(it.rawHeader || it.header, it.rawRows || it.sample); return Object.keys(sm).length ? <Note tone="ok">Sheet totals found: {Object.entries(sm).map(([k, v]) => `${k} ${v}`).join(" · ")} — the phone dashboard uses these as they are.</Note> : <Note tone="warn">No summary cells found (“SKU on dock:”, “Non urgent pallets on dock:”…) — the dashboard will count rows instead.</Note>; })()}
                <div className="flex items-center gap-3 mb-2"><p className="label-sm">3 · Preview</p><span className="text-xs" style={{ color: bad ? C.warn : C.muted }}>{preview.length} rows · {bad} with errors{bad ? " (skipped on apply)" : ""}</span><div className="flex-1" /><Primary onClick={apply} disabled={missingRequired.length > 0 || preview.length === 0}>{it.purpose === "Dock" ? "Apply as live dock data" : "Create / update products"}</Primary></div>
  ...
  return (
    <div style={bottomPad}>
      ...
      <div className="grid grid-cols-2 gap-2 px-5">
        ...
        <div className="rounded-2xl p-3.5" style={{ background: C.bg, border: `1px solid ${C.line}` }}><p className="text-xs" style={{ color: C.muted }}>Needed today<Lock /></p><p className="text-[26px] leading-tight font-semibold mt-0.5" style={{ color: C.bad }}>{sheetStats(s).blocked}</p><p className="text-[10px]" style={{ color: C.muted }}>blocking picking until inspected</p></div>
      ...
      <div className="px-5 pt-2">
        {tab === "blocked" && <div>
          <p className="text-xs py-2" style={{ color: C.muted }}>Dock pallets needed for picking — they won't move to the pick location until inspected.</p>
          {dockRowsLive(s).filter(r => r.blocking).map(b => { const prod = s.products.find(p => p.articleId === b.article); return (
            <button key={b.hu} onClick={() => prod && go("catalog", prod.id)} className="w-full text-left py-2.5" style={{ borderBottom: `1px solid ${C.line}` }}>
              <div className="flex items-center gap-2"><p className="text-sm flex-1 truncate font-medium">{b.name}</p><span className="text-xs px-2 py-0.5 rounded-md" style={{ background: C.bg, color: C.muted, border: `1px solid ${C.line}` }}>{b.location}</span></div>
              <p className="text-xs mt-0.5" style={{ color: C.muted }}>HU {b.hu} · arrived {b.arrivedTime} · {b.transporter}{prod ? "" : " · product not in catalog"}</p>
            </button>
          ); })}
...
function DockPresence({ product, onPickPallet, compact }) {
  ...
  if (!rows.length) return <div className="rounded-xl px-3 py-2 mb-2 text-xs flex items-center" style={{ background: C.bg, color: C.muted }}><Ic i={Truck} s={13} />Not on the docks right now.</div>;
  return (
    <div className="rounded-xl mb-2" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-2 px-3 py-2.5 text-left">
        ...
        <span className="text-sm flex-1"><b>On the docks now</b> — {pallets} pallet{pallets === 1 ? "" : "s"} · {rows.length} HU{rows.some(r => r.blocking) ? " · blocked for picking" : ""}</span>
      ...
      <div className="px-4 pt-3">
        ...
        {mode === "done" && completed && (
          <div className="rounded-2xl p-4" style={{ background: C.bg }}>
            ...
            <p className="text-sm mb-3">{s.products.find(p => p.id === completed.productId)?.name || "no product"} · {s.users.find(u => u.id === completed.controllerId)?.name} · {dayLabel(completed.completedAt)}, {hhmm(completed.completedAt)}{" · " + inspType(s, completed).name.toLowerCase()}</p>
...
function MChat({ s, set, user }) {
  ...
  const send = () => { if (!text.trim() || !conv) return; set(x => ({ ...x, conversations: x.conversations.map(c => c.id === conv.id ? { ...c, messages: [...(c.messages || []), { id: uid(), senderId: user.id, text: text.trim(), at: nowISO() }], lastRead: { ...(c.lastRead || {}), [user.id]: nowISO() } } : c) })); setText(""); };
  ...
  if (conv) return (
    <div className="flex flex-col" style={{ height: "100%" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3" style={{ background: C.bg }}>{(conv.messages || []).map(m => { const me = m.senderId === user.id; return <div key={m.id} className={`flex mb-1.5 ${me ? "justify-end" : "justify-start"}`}><div className="rounded-2xl px-3 py-2 max-w-[78%]" style={{ background: me ? C.accent : C.surface, color: me ? C.onDark : C.ink }}>{!me && conv.isGroup && <p className="text-[10px] font-medium" style={{ color: C.accent }}>{s.users.find(u => u.id === m.senderId)?.name}</p>}{m.productId && <p className="text-[10px] mb-0.5 px-1.5 py-0.5 rounded inline-block" style={{ background: me ? C.onDarkSoft : C.accentSoft, color: me ? C.onDark : C.accent }}>📦 {s.products.find(p => p.id === m.productId)?.name || "product"}</p>}<p className="text-sm">{m.text}</p><p className="text-[10px] mt-0.5" style={{ color: me ? C.onDarkMuted : C.muted }}>{hhmm(m.at)}</p></div></div>; })}</div>
      <div className="flex gap-2 p-3" style={{ borderTop: `1px solid ${C.line}` }}><button className="text-lg" style={{ color: C.muted }}><Ic i={Paperclip} s={18} mr={0} /></button><input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Message…" className="flex-1 text-sm rounded-full px-3 py-2 outline-none" style={{ ...inp, background: C.bg }} /><button onClick={send} className="w-9 h-9 rounded-full" style={{ background: C.accent, color: C.onDark }}><Ic i={Send} s={16} mr={0} /></button></div>
    </div>
  );
  return (
    <div>
...
function MNotifications({ s, set, user, go }) {
  ...
  return <div><TopBar title="Notifications" onBack={() => go("home")} /><div className="px-4">{mine.length === 0 ? <p className="text-sm py-8 text-center" style={{ color: C.muted }}>Quiet.</p> : mine.map(n => <button key={n.id} onClick={() => { read(n.id); if (n.entityType === "Inspection" && n.entityId) go("inspection", n.entityId); }} className="w-full text-left py-3" style={{ borderBottom: `1px solid ${C.line}` }}><p className="text-sm" style={{ fontWeight: n.readAt ? 400 : 600 }}>{n.message}</p><p className="text-xs" style={{ color: C.muted }}>{dayLabel(n.createdAt)}, {hhmm(n.createdAt)}</p></button>)}</div></div>;
}
